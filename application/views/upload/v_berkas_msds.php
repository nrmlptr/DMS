     ...
     <div class="content">
         ...
         <div class="block">
             <div class="block-header block-header-default">
                 <h3 class="block-title">Document MSDS & Safety Sign<small></small></h3>
             </div>
                 ...
                 <table class="table table-bordered table-striped table-vcenter js-dataTable-full">
                     ...
                     <tbody>
                         <?php
							...
							foreach ($berkas_msds->result() as $row) { ?>
                         <tr>
                             <td class="d-none d-sm-table-cell"><?= $loop++ ?></td>
                             <td class="d-none d-sm-table-cell"><?= $row->kd_berkas_msds ?></td>
                             <td class="d-none d-sm-table-cell"><?= $row->no_berkas_msds ?></td>
                             <td class="d-none d-sm-table-cell"><?= $row->judul_berkas_msds ?></td>
                             <td class="d-none d-sm-table-cell"><?= $row->keterangan_berkas_msds ?></td>
                             <td class="d-none d-sm-table-cell" align="center">
                                 <a href="<?= base_url('Unggah/downloadMSDS/' . $row->id_msds) ?>"
                                     class="btn btn-success" title="Download Document">
                                     <i class="fa fa-download"></i>
                                 </a>
				<?php if($this->session->userdata('akses') == 5){ ?>
                                 <a href="<?= base_url('Unggah/deleteMSDSById/' . $row->id_msds) ?>"
